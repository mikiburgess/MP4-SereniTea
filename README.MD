...
<p align="center">
    <img src="docs/images/serenitea-devices.png" alt="Illustration of the Serenitea Emporium site as viewed across a variety of devices">
</p>
...
- [Developments Since Initial Submission](#developments-_since_initial_submission)
...
#### User Stories
The user stories for SereniTea have been organised into 7 activity areas:
...
| User Story ID | As a ... | I want to be able to ... | So that I can ... |
|---------------|-----------|------------------------|-----------------|
| G.1 | Customer | Write a review for a product I have used (purchased or gifted) | Provide feedback to the seller and share my experience with others |
| G.2 | Customer | See all product reviews that I have written | Keep track of which products I've reviewed |
| G.3 | Customer | Edit a submitted review | Correct any mistakes or change my rating |
| G.4 | Customer | Delete a submitted review | Change my mind and remove my comments. |
| G.5 | Site Admin | Validate customer written product reviews before making them visible to all customers | Ensure reviews are appropriate and not fake or abusive, so that no inappropriate reviews are published to the site. |
...
Site administrators will also have access to pages for modifying the store catalogue.
...
<p align="center">
  <a href="docs/images/404-error-page.png">
  <img src="docs/images/404-error-page.png" alt="Bespoke 404 Page Not Found Error Page" width="400">
  </a>
  <br>
<em>Figure: 404 Error Page</em>
</p>
<br>
...
| 📂 ```Reviews``` | Enabling registered customers to review a product they have purchased or experienced (for example, received as a gift). |
...
| 📂 ```Static``` | Site stylesheets and icon |
| 📂 ```StaticFiles``` | Site stylesheets and icon, media files and all other resources gathered by WhiteNoise to enable Django to serve static files during deployment |
...
<br>
<p align="center">
  <a href="docs/design/colors/black-on-white.png">
...
### Data Management
The defined relational database schema was constructed in the online Postgres database [ElephantSQL](https://www.elephantsql.com/). The process of populating and updating the database was then supported by using the Django framework.
...
This section provides an overview of the SereniTea Emporium e-commerce web application functionality:
...
- *Product Reviews* - Customer submitted reviews of store products
...
Illustrations of these functionalities are also demonstrated in the [Walkthroughs](docs/WALKTHROUGHS.MD) and [Testing](docs/TESTING.MD) documents.
...
<br>
<p align="center">
  <img src="docs/images/item-search/desktop-search-bar.png" alt="desktop search facility" width="400">
<br>
...
<br>
<p align="center">
  <img src="docs/images/item-search/desktop-search-results.png" alt="desktop search results" width="400">
<br>
...
#### Improved Approach to Product Reviews
The initial version of this feature enables customers to submit a review for any product in the store. This therefore allows for reviews to be submitted for products they have purchased and received as gifts. However, this also currently allows for erroneous and malicious reviews to be submitted. In future this would be developed to limit the products customer can review, initially to only those they have purchased.

Other developments are also required to the product review feature. These were not implemented in this version due to time constraints and therefore being beyond project scope. However, an overview of the main required developments are covered below in the sections [Confirmation of product and review deletion](#confirmation_of_product_and_review_deletion) and [Product and Company Reviews](#product_and_company_reviews).
...
#### Confirmation of product and review deletion 
This feature is currently missing from the current version of this web application.

When a customer deletes a product review, or when a store administrator (system superuser) deletes a product from the store catalogue, clicking 'delete' should result in the user being asked 'Are you sure you want to delete this product/review?'. This is important to stop accidental deletions, as once deleted the data will be gone. 
...
To improve communication with visitors and customers, as well as customer engagement, the first significant development would be the improvement to the process of using customer-written product reviews, and allowing for customers to also review the company itself. This would include the following:
- Mechanism to notify site admin of customer-written reviews that require approval and providing a page where they can quickly review and mark reviews as approved or rejected, and remove any inappropriate reviews from the site.
- Approved reviews to be displayed on the relevant product pages, to assist future customers in their purchasing decisions.
- Notification email sent to customers to confirm their review has been submitted, plus an additional notification email to confirm when it has been reviewed and published ot the site.
- Dynamic calculation of product rating scores (stars) based on customer-entered ratings, rather than using the static star ratings currently stored in the backend database.
- Allow customers to upload images with their written reviews.
...
Due to not using the Code Institute online tools (due to the unreliability of Code Anywhere and loosing too many study days being unable to access my work, resulting in switching to developing in a local environment) support from the Code Institute tutors wasn't available to me. During the debugging process I therefore 
1. Sought assistance from my colleague, Dr N Caine, to see if deployment from their machine was also an issue. This was achieved by creating a git branch to use on their computer to test if their environment worked. It didn't. The same issues arose. (*Note: an accidental push from their machine ended up adding them as a contributor to this project. This was a one-off accident; they were not involved in the project work; Github history cah be reviewed to support this*)
2. Created a number of text branches for me to experiment with Django and associated libraries.

A compromise was reached for package incompatibilities by downgrading Django from version 5 to 4.2. 
...
#### CHALLENGE: Handling static files
Towards the end of development stage, this full stack site worked as intended when DEBUG was set to TRUE. However, once set to FALSE the site failed. Initially the failure related to being unable to load static files (stylesheets and images), then later (after some failed fixes) a 500 Server Error. This meant the site could not be deployed with Debug set to FALSE.

The settings used in the Module 4 examples (the Boutique Ado project) and the demonstrated approach to managing static files no longer work in later versions of Django. So, a new approach needed to be adopted, using [WhiteNoise](https://whitenoise.readthedocs.io/en/stable/django.html "Using WhiteNoise with Django").

#### The issue with static files
Following the guidance in the course material with a newer version of Django resulted in the following two errors:
- Lost access to all 'static' files, and
- Lost access to all images stored in the 'media' folder.

#### Fixing access to static files
To include all media images in the live site, the original 'media' folder was moved into the 'static' folder.

This ensured WhiteNoise would be able to handle them along with all other static files used across the site (css, js, etc).

Although all static files could remain in a single folder, the following line included in settings.py specified a different folder for use by WhiteNoise for managing static files during deployment, keeping the static files distinct from those used during development:
 ```STATIC_ROOT = BASE_DIR / "staticfiles" ```


Ahead of deployment to the live server, the static files need to be collated using the command
```python manage.py collectstatic```
Once the collation process is complete, all files can be accessed by the live deployed system (```DEBUG=False```), in deployment rather than development (```DEBUG=True```) mode.


Two final modifications needed to be made to the HTML code in order for Django to access the media files (site images).
For consistent images, those that are set and do not change, the following was used:
```{% static 'media/filename.jpg' %}```
For dynamic images, those selected at runtime according to data extracted from the Postgres database, the following alternative was employed:
```{% static 'media/' %}{{ product.image }}```
This latter enables the web pages to be served displaying the relevant product images, based on the current database content and user-selections.


#### Image Compression

An option when using WhiteNoise is to enable it to manage image compression. To allow for that feature the following code can be added to settings.py
...
However, I made the decision not to use WhiteNoise's compression facility as all site images have already been optimised for the web using TinyPNG. The following was therefore used instead, using Django's inbuilt storage management.

```STATICFILES_STORAGE = "django.contrib.staticfiles.storage.ManifestStaticFilesStorage"```


#### The Process
During the process of fixing the static files deployment issue, the relevant filenames were added to the development web pages so that the file paths could be checked and issues identified, to guide the development process.

<br>
<p align="center">
  <img src="docs/images/django/staticfile-name-check.png" alt="addition of filenames under site images" width="300">
<br>
<em>Figure: Example of displaying file paths in the dynamically generated HTML pages</em>
</p>
<br>


Django [logging](https://docs.djangoproject.com/en/5.0/topics/logging/ "Logging in Django") was also enabled to allow for a more detailed inspection of code behaviour. 


```
# DATA LOGGING DURING DEVELOPMENT (settings.py)
LOGGING = {
    "version": 1,
...
<br>
<p align="center">
  <img src="docs/images/django/django-logging.png" alt="django logging in action" width="400">
<br>
<em>Figure: Illustration of logging information during the initial stage, showing an early problem, where media files (images) were not being collected from the correct folder</em>
</p>
<br>
...
## Developments Since Initial Submission
In response to the feedback I received fom the initial project assessor, the following elements have been developed for the resubmission:

- **Product Reviews**: Addition of the *Reviews* app, including Model and Views and Forms (as discussed above and demonstrated through project Walkthroughs.)
- **Debug & Static Files**: Fixing the static files issue in order to set ```Debug = False``` (described in section [CHALLENGE: Handling static files](#challenge:_handling_static_files) )
- **Product Quantity**: Product quantities are restricted to be within the range 0-99.
- **Project Documentation**: Documentation updated in support of the *Reviews* app. This includes [User Stories](#user_stories), [TESTING.MD](docs/TESTING.MD), [WALKTHROUGHS.MD](docs/WALKTHROUGHS.MD) and [Future Features](#future_features).
...
- The Code Institute Project Assessor, whose informative and constructive feedback helped me improve this project.
